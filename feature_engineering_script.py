...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Feature engineering transaccional sin leakage (look-back only).")
    parser.add_argument("--input", type=Path, default=Path("credit_card_transactions.csv"), help="CSV de entrada")
    parser.add_argument("--output", type=Path, default=Path("credit_card_transactions_fe.csv"), help="CSV de salida")
...
def main() -> None:
    ...
    df_features = build_feature_engineered_dataset(df_raw)

    feature_cols = [c for c in df_features.columns if c.startswith(("A", "T", "L"))]
    ...
    print(f"Dataset enriquecido guardado en: {args.output.resolve()}")
